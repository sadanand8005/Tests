...
<div class="fieldcontain ${hasErrors(bean: commentInstance, field: 'user', 'error')} ">
	...
	<g:select id="user" name="user.id" from="${vogella.User.list()}" optionKey="id" value="${commentInstance?.user?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: commentInstance, field: 'feedback', 'error')} required">
	...
	<g:select id="feedback" name="feedback.id" from="${vogella.Feedback.list()}" optionKey="id" required="" value="${commentInstance?.feedback?.id}" class="many-to-one"/>
</div>
